...
def generate_frames():
    ...
    save_path = 'robot%d_shadow(ee)/' % robot_id
    os.makedirs(save_path, exist_ok=True)
    alpha = 0.5

    for i in range(500):
        print(i)
        real_robot_img = np.copy(plt.imread('real_results/1025_r%d/%d.jpeg'%(robot_id,i)).astype(float))/255

        img = plt.imread('robot%d_sim(ee)/%d.jpeg'%(robot_id,i))/255
...
def frames_2_video():
    img_array = []
    for i in range(500):
        filename = 'robot%d_shadow(ee)/%d.jpeg'%(robot_id,i)
        img = cv2.imread(filename)
        height, width, layers = img.shape
        size = (width, height)
        img_array.append(img)

    out = cv2.VideoWriter('robot_%d(ee).avi'%robot_id, cv2.VideoWriter_fourcc(*'DIVX'), 25, size)
